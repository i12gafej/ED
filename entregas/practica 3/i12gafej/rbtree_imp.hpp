    ...
    // Hint: use the recursive definition of a tree to unfold.
     ...
     if(token == "[]")
      ...
      if(token == "["){
        ...
        if(token == "B")
            tree->_root->set_color(RBTNode<T>::BLACK);
        else if(token == "R")
            tree->_root->set_color(RBTNode<T>::RED);
                             ...
                             typename RBTree<T>::Ref &tree)
{
    ...
    if(begin != end){
        tree->insert(data[(begin+end)/2]);
        create_inserting_median(data, begin, (begin+end)/2, tree);
        create_inserting_median(data, (begin+end)/2, end, tree);
    }
    //
}
...
typename RBTree<T>::Ref RBTree<T>::left() const
{
    ...
    subtree = RBTree<T>::create(_root->left());
    //
    return subtree;
}
...
typename RBTree<T>::Ref RBTree<T>::right() const
{
    ...
    // TODO
    // Hint: use the protected method to create a tree given the root node.
    subtree = RBTree<T>::create(_root->right());   
    //
    return subtree;
}
...
size_t compute_size(typename RBTree<T>::Ref const &tree)
{
    ...
    if(!tree->is_empty()){
        s = 1 + compute_size<T>(tree->left()) + compute_size<T>(tree->right());
    }
    //
    return s;
}
...
int compute_height(typename RBTree<T>::Ref const &tree)
{
    ...
    if(!tree->is_empty()){
        ...
        auto subtree_l = tree->left();
        auto subtree_r = tree->right();
        izquierda = compute_height<T>(subtree_l) + 1;
        derecha = compute_height<T>(subtree_r) + 1;  
...
bool RBTree<T>::is_a_binary_search_tree() const
{
    ...
    bool first = true;
    T prenum;
    auto p = [&first,&prenum](typename RBTNode<T>::Ref aux)mutable->bool{
        T value = aux->item();
        if(first){
            prenum=value;
            first=false;
...
bool RBTree<T>::meet_red_invariant() const
{
    ...
    auto lambda =  [](typename RBTNode<T>::Ref node){
        if(node->color()==RBTNode<T>::RED)
            return (node->left() == nullptr || node->left()->color() == RBTNode<T>::BLACK) && (node->right() == nullptr || node->right()->color() == RBTNode<T>::BLACK)
    };
    is_met = infix_process<T>(_root, lambda);
    //
    return is_met;
#endif
}
...
bool RBTree<T>::meet_black_invariant() const
{
    ...
    std::function<bool(typename RBTNode<T>::Ref, int )> check;
    bool first=true;
    int black=0;
    check=[&black, &first, &check](typename RBTNode<T>::Ref node, int n){
    ...
    if(node->left()!=nullptr){
        ret_val=ret_val&&check(node->left(),n);
    }
    if(node->right()!=nullptr){
        ret_val=ret_val&&check(node->right(),n);
    }
    return ret_val;
    };
    if(_root!=nullptr){
        is_met=check(_root,0);
    }
    //
    return is_met;
#endif
...
void RBTree<T>::insert(T const &k)
{
    ...
    if (!search(k))
    {
        ...
        if( this->is_empty() ){
            _root=RBTNode<T>::create(k);
            _current=_root;
        }
        else{
            _current = RBTNode<T>::create(k);
            if(_parent->item() > k)
...
template <class T>
RBTree<T>::RBTree(typename RBTNode<T>::Ref node)
{
    // TODO
    _root = node;
    //
