...
void SList<T>::insert(T const& new_it)
{
    ...
    {
        auto nuevo = SNode<T>::create(new_it, _actual->next());
        _actual->set_next(nuevo);
        _tam++;
    }
    //
...
void SList<T>::remove()
{
    ...
    else if(_actual->next() != nullptr)
    {
        auto ptr_anterior = _cabeza;
        while(ptr_anterior->next() != _actual)
        {
            ptr_anterior = ptr_anterior->next();
        }
        ptr_anterior->set_next(_actual->next());
        _actual = ptr_anterior->next();
    }
...
bool SList<T>::find(T const& it)
{
    ...
    while(_actual != nullptr and current() != it)
    {
        anterior = _actual;
        _actual = _actual->next();
    }
