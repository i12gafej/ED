...
BTree<T>::BTree ()
    ...
    {
        //TODO
        _root->create(it, nullptr, nullptr);        
        //
        assert(!is_empty());
        assert(item()==it);
...
template<class T>
  typename BTree<T>::Ref BTree<T>::create (std::istream& in) noexcept(false)
  {
      ...
      if(token == "["){
        in>>token;  
        T new_item;
        std::istringstream convert(token); //convert es el tipo istream que convierte el token en T
        convert >> new_item;               //volcamos en new_item el valor de T
        if(!convert)
          throw std::runtime_error("Wrong input format.");     
        tree->create_root(new_item);       //creamos una raiz con el valor 

        auto left_Subtree = BTree<T>::create(in); //creamos los arboles derecha e izquierda recursivamente
        auto right_Subtree = BTree<T>::create(in);
        tree->set_right(right_Subtree); //establecemos los arboles derechos e izquierdos anteriormente creados
...
template<class T>
  ...
  template<class T>
    void BTree<T>::create_root(const T& it)
    {
        assert(is_empty());
        //TODO
        _root=BTNode<T>::create(it); //las funcionnes con const y hay que declararlas como metodos del tipo
        //
        assert(!is_empty());
        assert(item()==it);
        assert(left()->is_empty());
